...
app.get('/api/config', (req, res) => res.json({ spotifyClientId: process.env.SPOTIFY_CLIENT_ID ?? null, shout: SHOUT }));
...
function afhandelen(ws, data) {
  switch (data.type) {
    ...
    case 'hl:game:start': {
      ...
      if (!started.ok) return fout(ws, 'Minimaal 2 spelers nodig');
